...
var ReplicaSetConnector = require('./connectors').ReplicaSetConnector;
...
function connect(uri) {
    // TODO - replica set connection with one node specified? Flag? 
    // I like flag in URI so that code does not need to change based on whether connection is 
    // single or replset
    var mongoURI = parseMongoURI(uri);
    if (mongoURI.hosts.length > 1 || (mongoURI.options && mongoURI.options.replicaSet)) {
        return _replicaSetConnect(mongoURI);
    } else {
 ...
 * @param {string} uri
 */
function _replicaSetConnect(uri) {
    var hosts = uri.hosts;
    var dbname = uri.db || "test";
    var uriOptions = uri.options || {};
    var serverOptions = _createServerConnectorOptions(uriOptions);
    ...
    var connector = new ReplicaSetConnector(hosts, serverOptions);
    
    var dbOptions = _createDatabaseOptions(uriOptions);
    var db = connector.getDB(dbname, dbOptions);
