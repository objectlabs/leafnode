...
function _singleNodeConnect(uri, options){
    ...
    var dbOptions = options["db"] || null;
    var mc = new MongoClient(server, dbOptions);
    try {
        var mcOpened = sync(mc, 'open');
        if (typeof mcOpened.db != "function") {
            console.log("open mc failed");
            console.log(JSON.stringify(mcOpened, null, 4));
            return null;
        }
        var db = mcOpened.db(uriObj.database);
