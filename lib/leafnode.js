...
function connect(uri, options) {
    ...
    if((uriObj.hosts.length > 1) || (uriObj.options && uriObj.options.replSet)){
        ...
        var server = new Server(host.host, host.port);
        var mc = new MongoClient(server);
        var db = sync(mc, 'open', server).db(uriObj.database);
